...
data$Ref = ifelse( data$Ref %in% c( "-" , " " ) , "" , data$Ref )
data$Alt = ifelse( data$Alt %in% c( "-" , " " ) , "" , data$Alt )
...
data$Sample = sapply( data$Sample , function(x){ paste( unlist( strsplit( x , "-" , fixed=TRUE ))[1:2] , collapse="." ) } )
